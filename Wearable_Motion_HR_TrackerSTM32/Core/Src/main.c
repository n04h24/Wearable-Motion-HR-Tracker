...
/* Private variables ---------------------------------------------------------*/
I2C_HandleTypeDef hi2c1; // EXTERNAL
...
int main(void)
{
  ...
  void config_I2Cmem(uint16_t device, uint16_t memory_add, uint8_t write_mem, uint16_t mem_size, uint16_t data_size) {
	  ...
	  status = HAL_I2C_Mem_Read(&hi2c1, device << 1, memory_add, mem_size, &check_mem, data_size, 100);
	  //Read memory just written-to
	  if (status == HAL_OK){
		  snprintf(UART, sizeof(UART), "New register 0x%02X value is 0x%02X\n", memory_add, check_mem);
		  //
		  HAL_UART_Transmit(&huart2, (uint8_t*)UART, strlen((char*)UART), 100);
		  //
	 	  }
  ...
  //TEST X(1)
  HAL_I2C_Mem_Read(&hi2c1, MPU6050 << 1, 0x0D, I2C_MEMADD_SIZE_8BIT, &check_mem, 1, 100);
	  //Read Self-Test X
  ...
  //TEST Y(1)
  HAL_I2C_Mem_Read(&hi2c1, MPU6050 << 1, 0x0E, I2C_MEMADD_SIZE_8BIT, &check_mem, 1, 100);
	  //Read Self-Test X
  ...
  //TEST Z(1)
  HAL_I2C_Mem_Read(&hi2c1, MPU6050 << 1, 0x0F, I2C_MEMADD_SIZE_8BIT, &check_mem, 1, 100);
	  //Read Self-Test X
  ...
  //TEST XYZ(2)
	HAL_I2C_Mem_Read(&hi2c1, MPU6050 << 1, 0x10, I2C_MEMADD_SIZE_8BIT, &check_mem, 1, 100);
	  //Read Self-Test X
	snprintf(UART, sizeof(UART), "Self-Test Z-value is 0x%02X\n", check_mem);
		  //Format Self-Test X (string)
	HAL_UART_Transmit(&huart2, (uint8_t*)UART, strlen((char*)UART), 100);
